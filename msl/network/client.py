...
class Client(Network, asyncio.Protocol):
    ...
    def admin_request(self, attrib, **parameters):
        ...
        parameters : :obj:`dict`
            The parameters to send to the

        Returns
        -------
        The reply from the Network :class:`~msl.network.manager.Manager`.
        """
        result = self._send_request_blocking(None, attrib, parameters)
    ...
    def _request(self, *args, **kwargs):
        ...
        result = self._send_request_blocking(self._service, self._attribute, kwargs)
        if result['error']:
            # self.raise_latest_error()
